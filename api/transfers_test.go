...
func randomTransfer(fromID, toID int64) db.Transfer {
	return db.Transfer{
...
func TestGetTransferAPI(t *testing.T) {
	user1, _ := randomUserAndPassword()
	user2, _ := randomUserAndPassword()
	...
	testCases := []struct {
		...
		{
			name:       "InternalError (From Account)",
			transferID: transfer.ID,
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationTypeBearer, user1.Username, time.Minute)
			},
...
func TestListTransfersAPI(t *testing.T) {
	...
	for range 10 {
		transfers = append(transfers, randomTransfer(account1.ID, account2.ID))
	}
	...
	testCases := []struct {
	...
	}{
		{
			...
			query: url.Values{
				"page":       []string{"2"},
				"size":       []string{"5"},
				"account_id": []string{fmt.Sprint(account1.ID)},
			},
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationTypeBearer, user1.Username, time.Minute)
			},
		...
		},
		{
			...
			query: url.Values{
				"page":       []string{"2"},
				"size":       []string{"5"},
				"account_id": []string{fmt.Sprint(account2.ID)},
			},
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationTypeBearer, user2.Username, time.Minute)
			},
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					...
					Times(0)
				store.EXPECT().
					ListAccountTransfers(gomock.Any(), gomock.Any()).
					Times(0)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusBadRequest, recorder.Code)
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					GetAccount(gomock.Any(), gomock.Eq(account1.ID)).
					Times(1).
					Return(account1, sql.ErrConnDone)
				store.EXPECT().
					ListAccountTransfers(gomock.Any(), gomock.Any()).
					Times(0)
			},
...
func TestCreateTransfersAPI(t *testing.T) {
	user1, _ := randomUserAndPassword()
	user2, _ := randomUserAndPassword()
	user3, _ := randomUserAndPassword()
	currency := util.USD
	account1 := randomAccountWithMinAmountAndCurrency(user1.Username, math.MaxInt64/2, currency)
	account2 := randomAccountWithMinAmountAndCurrency(user2.Username, math.MaxInt64/2, currency)
	uncompatibleAccount := randomAccountWithMinAmountAndCurrency(user3.Username, math.MaxInt64/2, util.EUR)

	amount := decimal.NewFromInt(util.RandomInt(0, 9999))
	...
	testCases := []struct {
	...
	}{
		{
			...
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusOK, recorder.Code)
				requireBodyMatchTransferTx(t, recorder.Body, result)
			},
		},
		{
			name: "Unauthorized",
		...
		{
			name: "BadRequest (Account Currency Mismatch)",
			body: gin.H{
				"from_account_id": account1.ID,
				"to_account_id":   uncompatibleAccount.ID,
				"Amount":          amount,
				"Currency":        currency,
			},
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					...
					Return(account1, nil)
				store.EXPECT().
					GetAccount(gomock.Any(), gomock.Eq(uncompatibleAccount.ID)).
					Times(1).
					Return(uncompatibleAccount, nil)
				store.EXPECT().
					TransferTx(gomock.Any(), gomock.Any()).
					Times(0)
			},
